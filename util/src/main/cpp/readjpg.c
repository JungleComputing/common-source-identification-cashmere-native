...
int read_JPEG_file (unsigned char *buffer, const char *filename)
{
...
JNIEXPORT jint JNICALL Java_nl_junglecomputing_common_1source_1identification_ReadJPG_readJPG
(JNIEnv *env, jclass c, jobject buffer_object, jstring fileName) {
  jboolean copied;
  const char *filename = env->GetStringUTFChars(fileName, &copied);

  jbyte *buffer = (jbyte *) env->GetDirectBufferAddress(buffer_object);

  jint result = read_JPEG_file ((unsigned char *) buffer, filename);
